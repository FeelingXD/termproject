...
$query = "SELECT SEQ ,ID, CONTENTS, WRITER ,title
FROM(
    SELECT SEQ, ID, CONTENTS, WRITER , title
    FROM(
        SELECT ROWNUM AS SEQ, ID, CONTENTS, WRITER, title
        FROM(
            SELECT *
    ...
    <table class="style1">
        ...
        <tbody>
            <tr text align="center">
                <!-- 내용 첨부할것 -->
                <td text><?= $board['ID']?></td>
                <td text><a href="view_notice.php?id=<?=$board['ID']?>"><?= $board['TITLE']?></a></td>
                <td text><?= $board['WRITER']?></td>
            </tr>
        </tbody>
    <?php
            }
